...
export const updateTour = createAsyncThunk(
  ...
  async ({ id, tourData }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token || localStorage.getItem('accessToken');

      const config = {
        headers: {
          Authorization: `Bearer ${token}`,
          // Don't set Content-Type manually for FormData; axios does it automatically
        },
      };

      const response = await axios.patch(`${API_URL}/tours/${id}`, tourData, config);
      return response.data.data; // Return the updated tour
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTour.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        // Update the specific tour in the local array
